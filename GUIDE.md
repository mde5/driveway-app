...
The map is in `MapView.tsx` and is dynamically imported with `ssr: false` because Google Maps requires browser APIs. The `APIProvider` from `@vis.gl/react-google-maps` loads the Maps JS API, and `Marker` components render a pin for each listing.
...
The hours input calculates the total in real time: hourly rate × hours, switching to the day rate at 8+ hours.
